...
exports = module.exports = function() {
  ...
  httpGetFile = function httpGet(iUrl, iFile, iCB) {

    // Check vars
    if(!iUrl || !iFile) return false;

    if(typeof iCB !== 'function') {
      var iCB = function(err) { return err; };
    }

    // Get and save
    mHTTP.get(iUrl, function(res) {
      if(res.statusCode == "200") {
        res.pipe(mFS.createWriteStream(iFile));
        return iCB(null);
      } else {
        res.destroy();
        return iCB("Bad response: " + res.statusCode);
      }
    }).on('error', function(e) {
      return iCB("Error: " + e.message);
    });
  };
